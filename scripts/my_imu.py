...
data = []
my_imu = Imu()
my_imu.header.frame_id = "imu_link"
...
imu_pub = rospy.Publisher("my_imu_topic", Imu, queue_size=10)
...
while True:
	try:
		data = read_data()
		my_imu.orientation.x = 0
		my_imu.orientation.y = 0
		my_imu.orientation.z = 0
		my_imu.orientation.w = 1
		my_imu.angular_velocity.x = float(data[3])
		my_imu.angular_velocity.y = float(data[4])
		my_imu.angular_velocity.z = float(data[5])
		my_imu.linear_acceleration.x = float(data[0])
		my_imu.linear_acceleration.y = float(data[1])
